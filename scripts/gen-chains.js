...
// PUBLISH BOTH SPELLINGS. The spec-conformant hashed form is the canonical key;
// the raw chain-id spelling (`cosmos:kava_2222-10`) -- what every wallet, explorer
// and human actually writes -- is recorded as an alias that also resolves. A
// lookup in either dialect lands on the same chain. An alias can never collide
// with a conformant reference: it only exists when the raw id contains a
// character the direct grammar forbids, and it can never look like a hashed key
// (validate refuses a raw id that starts with "hashed-").
const aliasesFor = (caip2, chainId) => (caip2.startsWith('cosmos:hashed-') ? [`cosmos:${chainId}`] : []);
...
for (const [caip2, e] of Object.entries(existing)) if (e.curated) chains[caip2] = { ...e, coingeckoPlatform: platformFor(caip2, e.chainId), aliases: aliasesFor(caip2, e.chainId) };
...
for (const c of registry) {
  ...
  chains[caip2] = {
    namespace: 'cosmos',
    chainId: c.chainId,
    aliases: aliasesFor(caip2, c.chainId),
    name: c.prettyName,
    ...
    evidence,
    note: hashed ? `chain_id "${c.chainId}" is not a legal direct CAIP-2 reference under the cosmos profile, so the canonical key is the hashed form; the raw spelling cosmos:${c.chainId} is published as an alias and resolves to this row.` : null,
  };
}
