...
| — linked to a CoinGecko coin | 3,688 (63.9%) |
|   rule-proposed / model-inferred / human-accepted | 1,929 / 1,759 / 0 |
| — chain identified | 1,104 (19.1%) |
...
- `related.dtiEquivalent` — coins the DTI registry declares functionally fungible
  with this one. **This is currently always empty, and the reason is worth
  knowing.** DTIF's registry does maintain functionally-fungible groups, which
  is exactly the structure you would want in order to enumerate "every chain
  USDT is on". The free monthly snapshot does not publish group membership, so
  we cannot read it. The field is kept in the schema because the grouping is
  real and we would populate it immediately if the data became available.
