...
for (const rec of dtiTokens) {
  ...
  // `typeLabel` is inferred by this project, not published by the standard, so
  // it must not sit beside genuine registry fields: DTIF's terms forbid
  // modifying the Registry "in any way which could be misleading to any
  // person", and a reader has to be able to tell which fields are theirs.
